...
class TestSudoRelayDeviceState:
    def test_off_when_no_session(self):
        from unseen_university.devices.sudo_relay.device import SudoRelayDevice

        device = SudoRelayDevice()
        with patch("unseen_university.devices.sudo_relay.device._session_exists", return_value=False):
    ...
    def test_processing_when_pending_sh_exists(self, tmp_path):
        ...
        pending.touch()
        device = SudoRelayDevice()
        with patch("unseen_university.devices.sudo_relay.device._session_exists", return_value=True), \
             patch("unseen_university.devices.sudo_relay.device._PENDING_SH", pending):
    ...
    def test_needpw_when_password_prompt_visible(self, tmp_path):
        ...
        device = SudoRelayDevice()
        with patch("unseen_university.devices.sudo_relay.device._session_exists", return_value=True), \
             patch("unseen_university.devices.sudo_relay.device._PENDING_SH", tmp_path / "nope"), \
             patch("unseen_university.devices.sudo_relay.device._pane_text", return_value="[sudo] password for"):
    ...
    def test_waiting_when_session_running_no_pending(self, tmp_path):
        ...
        with patch("unseen_university.devices.sudo_relay.device._session_exists", return_value=True), \
             patch("unseen_university.devices.sudo_relay.device._PENDING_SH", tmp_path / "nope"), \
             patch("unseen_university.devices.sudo_relay.device._pane_text", return_value="watching..."):
...
class TestSudoRelayDeviceChat:
    ...
    def test_pw_command_not_logged(self):
        ...
        device = SudoRelayDevice()
        log_calls = []
        with patch("unseen_university.devices.sudo_relay.device.log") as mock_log, \
             patch("unseen_university.devices.sudo_relay.device._session_exists", return_value=True), \
             patch("unseen_university.devices.sudo_relay.device._pane_text", return_value="[sudo] password:"), \
             patch("unseen_university.devices.sudo_relay.device.subprocess.run", return_value=MagicMock(returncode=0)):
...
class TestSudoRelayShim:
    def test_start_registers_with_skeleton(self):
        from unseen_university.devices.sudo_relay.shim import SudoRelayShim
    ...
    def test_self_test_passed_when_waiting(self, tmp_path):
        ...
        with patch("unseen_university.devices.sudo_relay.device._session_exists", return_value=True), \
             patch("unseen_university.devices.sudo_relay.device._PENDING_SH", tmp_path / "nope"), \
             patch("unseen_university.devices.sudo_relay.device._pane_text", return_value="waiting..."):
            result = shim.self_test()
