...
from lab.competition.classifiers.llm_classifier import (  # noqa: E402
    FALLBACK_TYPE,
    _cache_get,
    _cache_put,
    _ensure_cache_table,
...
class TestLlmClassifierCache(unittest.TestCase):
    ...
    def test_returns_string_memory_type(self):
        with patch("lab.competition.classifiers.llm_classifier._call_llm", return_value="FACTUAL"):
    ...
    def test_returns_one_cloud_call_on_cache_miss(self):
        with patch("lab.competition.classifiers.llm_classifier._call_llm", return_value="PROCEDURAL"):
            mtype, cloud = classify(self.test_text)
        self.assertEqual(cloud, 1)
        self.assertEqual(mtype, "PROCEDURAL")

    def test_second_call_returns_zero_cloud_calls(self):
        with patch("lab.competition.classifiers.llm_classifier._call_llm", return_value="EPISODIC") as mock_llm:
    ...
    def test_fallback_when_llm_returns_none(self):
        with patch("lab.competition.classifiers.llm_classifier._call_llm", return_value=None):
            mtype, cloud = classify(self.test_text)
    ...
    def test_fallback_result_not_cached(self):
        ...
        with patch("lab.competition.classifiers.llm_classifier._call_llm", return_value="CONCEPTUAL") as mock_llm:
