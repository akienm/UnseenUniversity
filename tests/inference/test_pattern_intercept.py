...
class TestKeywords:
    def test_extracts_words(self):
        from devices.inference.pattern_intercept import _keywords
    ...
    def test_similarity_identical(self):
        from devices.inference.pattern_intercept import _similarity
...
class TestFindPatternMatch:
    ...
    def test_returns_none_when_no_rows(self):
        from devices.inference.pattern_intercept import find_pattern_match
...
class TestTryIntercept:
    ...
    def test_returns_none_when_no_match(self):
        ...
        with patch("devices.inference.pattern_intercept.find_pattern_match", return_value=None):
            result = try_intercept(self._req())
        assert result is None

    def test_returns_cached_response_on_match(self):
        from devices.inference.pattern_intercept import try_intercept, PatternMatch
        match = PatternMatch(
            pattern_id=42, pattern_text="start stop rack", response_text="Use BaseDevice.",
            hit_count=7, similarity=0.6,
        )
        with patch("devices.inference.pattern_intercept.find_pattern_match", return_value=match):
            with patch("devices.inference.pattern_intercept.record_hit"):
                result = try_intercept(self._req())
        assert result is not None
    ...
    def test_records_hit_on_match(self):
        ...
        recorded = []
        with patch("devices.inference.pattern_intercept.find_pattern_match", return_value=match):
            with patch("devices.inference.pattern_intercept.record_hit", side_effect=lambda pid, **kw: recorded.append(pid)):
    ...
    def test_returns_none_for_very_short_query(self):
        ...
        with patch("devices.inference.pattern_intercept.find_pattern_match") as mock_find:
