...
def cleanup_test_creds():
    ...
    from unseen_university.devices.vault.store import delete_credential
...
def test_encrypt_decrypt_round_trip(tmp_path):
    ...
    with patch("unseen_university.devices.vault.store._MASTER_KEY_PATH", key_file):
        from unseen_university.devices.vault.store import decrypt, encrypt
...
def test_encrypt_produces_different_ciphertexts(tmp_path):
    ...
    key_file = tmp_path / "master.key"
    with patch("unseen_university.devices.vault.store._MASTER_KEY_PATH", key_file):
        from unseen_university.devices.vault.store import encrypt
...
def test_upsert_and_get_credential():
    from unseen_university.devices.vault.store import get_credential, upsert_credential
...
def test_delete_credential():
    from unseen_university.devices.vault.store import delete_credential, get_credential, upsert_credential
    upsert_credential(_TEST_OWNER, _TEST_KEY, _TEST_VALUE, [_TEST_DEVICE])
...
def test_list_credentials_includes_upserted():
    from unseen_university.devices.vault.store import list_credentials, upsert_credential
    upsert_credential(_TEST_OWNER, _TEST_KEY, _TEST_VALUE, [_TEST_DEVICE])
...
def test_missing_credential_returns_empty_string():
    from unseen_university.devices.vault.store import get_credential
...
def test_get_credential_degrades_when_db_unavailable():
    ...
    with patch("unseen_university.devices.vault.store._connect", side_effect=Exception("DB down")):
        from unseen_university.devices.vault.store import get_credential
        result = get_credential(_TEST_DEVICE, _TEST_OWNER, _TEST_KEY)
    assert result == ""
...
def test_client_get_credential_degrades():
    ...
    with patch("unseen_university.devices.vault.store.get_credential", side_effect=RuntimeError("boom")):
...
def test_admin_password_set_and_verify(tmp_path):
    from unseen_university.devices.vault.store import set_admin_password, verify_admin_password
...
def test_admin_session_create_and_validate():
    from unseen_university.devices.vault.store import create_admin_session, validate_admin_session, revoke_admin_session
...
def test_invalid_session_token_rejected():
    from unseen_university.devices.vault.store import validate_admin_session
