...
def _patch_cortex(monkeypatch):
    ...
    cortex = _make_cortex()
    monkeypatch.setattr(
        "unseen_university.devices.igor.tools.receive_cc_direction._get_cortex",
        lambda: cortex,
    )
    return cortex
...
def test_empty_content_skipped():
    from unseen_university.devices.igor.tools.receive_cc_direction import receive_cc_direction
