...
class TestPGDatabaseProxySearchPath:
    ...
    def test_default_search_path(self):
        with patch("psycopg2.pool.ThreadedConnectionPool"):
            from unseen_university.devices.igor.memory.db_proxy import PGDatabaseProxy
...
class TestFactorySearchPaths:
    ...
    @patch.dict("os.environ", {"UU_HOME_DB_URL": "postgresql://fake",
                               "IGOR_HOME_SEARCH_PATH": "", "IGOR_LOCAL_SEARCH_PATH": ""})
    @patch("psycopg2.pool.ThreadedConnectionPool")
    def test_home_proxy_excludes_instance(self, mock_pool):
        from unseen_university.devices.igor.memory.db_proxy import make_home_proxy
    ...
    def test_local_proxy_includes_all(self, mock_pool):
        from unseen_university.devices.igor.memory.db_proxy import make_local_proxy
    ...
    def test_infra_proxy_infra_only(self, mock_pool):
        from unseen_university.devices.igor.memory.db_proxy import make_infra_proxy
...
class TestPGSchemaBootstrap:
    ...
    def test_pg_schema_creates_schemas(self):
        from unseen_university.devices.igor.memory.cortex import _PG_SCHEMA
