...
class TestPostToCcInboxBridge:
    def test_forwards_args_to_append(self):
        from unseen_university.devices.igor.cognition import cc_inbox_bridge

        with patch("devlab.claudecode.cc_inbox.append") as mock_append:
            cc_inbox_bridge.post_to_cc_inbox(
                kind="test_kind",
    ...
    def test_non_fatal_on_append_exception(self):
        ...
        with patch("devlab.claudecode.cc_inbox.append", side_effect=RuntimeError("boom")):
            # Must not raise
