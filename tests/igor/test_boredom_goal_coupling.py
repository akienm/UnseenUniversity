...
from unseen_university.devices.igor.cognition.push_sources import BoredomSource  # noqa: E402
...
def test_surface_empty_when_no_goals():
    ...
    src = BoredomSource()
    with patch("unseen_university.devices.igor.tools.goal_graph._fetch_goal_facia", return_value=[]):
...
def test_surface_empty_when_all_dormant():
    ...
    with patch("unseen_university.devices.igor.tools.goal_graph._fetch_goal_facia", return_value=goals):
        out = src._surface_active_goals(cortex)
    assert out == []
    assert not cortex.twm_push.called
...
def test_surface_degrades_on_fetch_failure():
    ...
    src = BoredomSource()
    with patch(
        "unseen_university.devices.igor.tools.goal_graph._fetch_goal_facia",
