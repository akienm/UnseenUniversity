...
from wild_igor.igor.env_sync import (
    SAFETY_GATE_NAMES,
    _is_safety_gate,
    hydrate_from_graph,
    push_vars_to_graph,
...
class TestPushExcludesSafetyGates:
    def test_push_skips_tier5(self):
        cortex = MagicMock()
        with patch("wild_igor.igor.env_sync._ensure_root_nodes", lambda c: None):
...
class TestHydrateRefusesSafetyGates:
    def _make_cortex_with_safety_gate_node(self, key, value):
        ...
        def _get_children(cat_id):
            ...
            from wild_igor.igor.env_sync import SYSCFG_FEATURES_ID
