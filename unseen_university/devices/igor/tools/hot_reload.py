...
_BLOCKED_PREFIXES = (
    "unseen_university.devices.igor.brainstem",
    "unseen_university.devices.igor.memory.models",  # dataclass defs — isinstance breaks on reload
    "unseen_university.devices.igor.memory.cortex",  # owns live DB proxy
    "unseen_university.devices.igor.cognition.reasoners.base",
    "unseen_university.devices.igor.tools.registry",  # registry itself — would wipe all tools
    "unseen_university.devices.igor.tools.hot_reload",  # this module
)
...
def list_loaded_modules() -> str:
    ...
    modules = sorted(
        ...
        if k.startswith("unseen_university.devices.igor") and sys.modules[k] is not None
    )
    if not modules:
...
registry.register(
    Tool(
        ...
        description=(
            ...
            "Example: reload_module('unseen_university.devices.igor.tools.filesystem')"
        ),
        parameters={
            "type": "object",
            "properties": {
                "module_name": {
                    "type": "string",
                    "description": "Dotted module name, e.g. 'unseen_university.devices.igor.tools.filesystem'",
                },
            },
