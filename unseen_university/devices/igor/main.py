...
def _deposit_prediction_error(
    ...
    try:
        ...
        try:
            log_error(
                kind="BARE_EXCEPT",
                detail=f"_deposit_prediction_error: {_exc_outer}",
            )
        except Exception as _exc:
            from .cognition.forensic_logger import log_error as _le
...
class Igor(IgorBase):
    ...
    def _push_state_inventory(self) -> None:
        ...
        try:
            ...
            for r in rows:
                try:
                    _raw = r["metadata"]
                    meta = _raw if isinstance(_raw, dict) else _json.loads(_raw or "{}")
                except Exception:
                    meta = {}
    ...
    def execute_habit(self, habit_id: str, args: dict) -> dict:
        ...
        try:
            if _habit_type == "question":
                ...
                from unseen_university.devices.igor.tools.registry import registry as _tool_registry
        ...
        try:
            ...
            if response_text:
                ...
                if _tool_name:
                    ...
                    try:
                        from unseen_university.devices.igor.tools.registry import registry as _bg_tool_reg
    ...
    def _process_inner(
        ...
        if author == "claude-code" and user_input.startswith("CC:"):
            ...
            try:
                from unseen_university.devices.igor.tools.registry import registry as _cc_registry
        ...
        if response_text and not _response_from_tool_dispatch:
            ...
            if _tool_name:
                ...
                response_text = _cleaned
                try:
                    from unseen_university.devices.igor.tools.registry import registry as _tool_reg
...
""")
    ...
    def _tier0_response(
        ...
        if any(p in t for p in _tool_words):
            try:
                from unseen_university.devices.igor.tools.registry import registry as _reg
    ...
    def _shutdown(self, reason: str = "shutdown"):
        ...
        # NOTE: sys.modules flush removed — the bash restart loop (exit 42)
        # spawns a fresh Python process, so modules are already clean.
        # Flushing here risks AttributeError in atexit/signal handlers that
        # still hold references to flushed module objects.
