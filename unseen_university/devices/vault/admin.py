...
CLI usage (set / change admin password):
    python3 -m unseen_university.devices.vault.admin set-password
"""
...
def handle_login(body: dict) -> tuple[int, dict]:
    ...
    from unseen_university.devices.vault.store import create_admin_session, verify_admin_password
...
def handle_logout(token: str) -> tuple[int, dict]:
    ...
    from unseen_university.devices.vault.store import revoke_admin_session
...
def handle_list(token: str, owner_filter: str | None = None) -> tuple[int, dict | list]:
    ...
    from unseen_university.devices.vault.store import list_credentials
...
def handle_upsert(token: str, body: dict) -> tuple[int, dict]:
    ...
    from unseen_university.devices.vault.store import upsert_credential
...
def cli_set_password() -> None:
    ...
    if len(pw1) < 8:
        ...
        sys.exit(1)
    from unseen_university.devices.vault.store import set_admin_password
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "set-password":
        ...
        print("Usage: python3 -m unseen_university.devices.vault.admin set-password", file=sys.stderr)
        sys.exit(1)
