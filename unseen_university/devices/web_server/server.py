...
def _deliver_to_tmux(content: str, sender: str, channel: str) -> None:
    ...
    try:
        from unseen_university.devices.claude.tmux_face import send_to_session
    except ImportError:
        return
...
async def _api_sudo_relay_feed(request: Request):
    ...
    from unseen_university.config.device_config import unseen_university_home
...
def _nanny_cron_backend():
    from unseen_university.devices.nanny.cron_backend import get_cron_backend
...
async def _api_hubert_audit_flags(request: Request):
    ...
    """
    try:
        from unseen_university.devices.hubert.repo_auditor import read_flags
...
async def _api_hubert_audit_flag_review(request: Request):
    ...
    try:
        ...
        from unseen_university.devices.hubert.repo_auditor import review_flag
...
def _get_feeds_imap():
    ...
    with _feeds_imap_lock:
        if _feeds_imap is None:
            try:
                from unseen_university.devices.bus.imap_server import IMAPServer, _TEST_MODE
...
async def _api_vault_login(request: Request):
    ...
    try:
        ...
        return JSONResponse({"error": "invalid JSON"}, status_code=400)
    try:
        from unseen_university.devices.vault.admin import handle_login
...
async def _api_vault_logout(request: Request):
    ...
    try:
        from unseen_university.devices.vault.admin import handle_logout
...
async def _api_vault_credentials_get(request: Request):
    ...
    try:
        from unseen_university.devices.vault.admin import handle_list
...
async def _api_vault_credentials_put(request: Request):
    ...
    try:
        ...
        return JSONResponse({"error": "invalid JSON"}, status_code=400)
    try:
        from unseen_university.devices.vault.admin import handle_upsert
...
async def _api_vault_credentials_delete(request: Request):
    ...
    try:
        ...
        return JSONResponse({"error": "invalid JSON"}, status_code=400)
    try:
        from unseen_university.devices.vault.admin import handle_delete
