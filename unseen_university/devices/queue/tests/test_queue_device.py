...
class TestQueueDeviceContract:
    ...
    @pytest.fixture
    def device(self):
        with patch("unseen_university.devices.queue.device._db_conn") as mock_conn:
...
class TestQueueNextGateTripped:
    def test_returns_none_when_gate_tripped(self, tmp_path):
        ...
        dev_mod.GATE_FILE = gate_file
        try:
            with patch("unseen_university.devices.queue.device._db_conn"):
                dev = QueueDevice()
...
class TestQueueNextMocked:
    ...
    def device_no_gate(self, tmp_path):
        ...
        original = dev_mod.GATE_FILE
        dev_mod.GATE_FILE = gate_file
        with patch("unseen_university.devices.queue.device._db_conn") as mock_conn_fn:
            conn = MagicMock()
