...
class GrannyShim(BaseShim):
    ...
    def _get_daemon(self):
        ...
        from unseen_university.devices.granny.daemon import run_loop
    ...
    def health_surface(self) -> dict:
        ...
        try:
            from unseen_university.devices.granny.daemon import get_daemon
            daemon = get_daemon()
        ...
        try:
            ...
            subprocess.run(
                ["tmux", "send-keys", "-t", _TMUX_SESSION,
                 f"{venv_python} -m unseen_university.devices.granny.daemon", "Enter"],
                check=True,
            )
