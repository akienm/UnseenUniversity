...
Storage: ~/.TheIgors/Igor-wild-0001/response_habituation.json
...
_STOP_WORDS = frozenset({
    ...
    "in", "it", "that", "this", "for", "on", "with", "be", "have", "do", "at",
    "by", "from", "as", "but", "not", "you", "we", "they", "he", "she",
})
...
class ResponseHabituation:
    ...
    def top_habituated(self, n: int = 20) -> list[tuple[str, int, float]]:
        ...
        now    = time.time()
        ranked = [
            (w, e["count"], self.decay_factor(w, now))
            for w, e in self._store.items()
        ]
