"""
hebbian_bridge.py — T-308: Hebbian bridge between word graph and memory graph.
...
def wg_boost_search(
    ...
    try:
        ...
        if boosts:
            forensic.debug(
                "[hebbian_bridge] wg_boost: %d/%d candidates boosted from %d predictions",
                len(boosts),
                len(candidates),
                len(predictions),
            )
        ...
        log.debug("[hebbian_bridge] wg_boost_search error: %s", e)
        return {}
...
def record_retrieval_boost(word_graph, memory, arousal: float) -> None:
    ...
    try:
        ...
        forensic.debug(
            "[hebbian_bridge] retrieval_boost: memory=%s importance=%.2f"
            " arousal=%.2f boost=%.3f",
            memory.id,
            importance,
            arousal,
        ...
        log.debug("[hebbian_bridge] record_retrieval_boost error: %s", e)
...
def wg_predict_for_activation(word_graph, activated: list) -> set:
    ...
    try:
        ...
        if predicted:
            forensic.debug(
                "[hebbian_bridge] spread_wg: %d predicted words from %d activated nodes",
                len(predicted),
                min(len(activated), 5),
            )
        return predicted
        ...
        log.debug("[hebbian_bridge] wg_predict_for_activation error: %s", e)
        return set()
