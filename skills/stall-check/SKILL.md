...
```bash
python3 "${UU_ROOT:-$HOME/dev/src/UnseenUniversity}/devlab/claudecode/stall_check.py" "$@" 2>/dev/null
```

Or with `--all`:
```bash
python3 "${UU_ROOT:-$HOME/dev/src/UnseenUniversity}/devlab/claudecode/stall_check.py" --all 2>/dev/null
```
...
- "Reset to sprint": `python3 ${UU_ROOT:-$HOME/dev/src/UnseenUniversity}/devlab/claudecode/cc_queue.py reset <id>`
- "Move to hold": `python3 ${UU_ROOT:-$HOME/dev/src/UnseenUniversity}/devlab/claudecode/cc_queue.py block <id> "stalled in_progress: check status"`
