...
**Target modes:** `WORKING` = the current local HEAD, *pushed or not* (daily driver: commit locally → prove → push). `BRANCH <name>` = a named branch's tip. OPEN: (i) whether `HEAD` is a distinct mode meaning the origin/pushed tip — making the trio pushed/local/branch and adding a remote dependency — or just a synonym for the WORKING default; (ii) BRANCH baseline = merge-base-with-main vs tip~1. Red baseline for WORKING = parent (HEAD~1).
