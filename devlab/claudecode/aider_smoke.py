...
# The self-contained capability task: a TTLCache stub + a deterministic failing test.
# Time is injectable (time_fn) so expiry is testable without real sleeps.
STUB = '''\
...
def build_sandbox() -> Path:
    ...
    (SANDBOX / "ttl_cache.py").write_text(STUB)
    (SANDBOX / "test_ttl_cache.py").write_text(TEST)
    ...
    _run(["git", "commit", "-qm", "sandbox: RED (TTLCache stub + failing tests)"], cwd=SANDBOX)
    return SANDBOX


def run_aider(model: str) -> float:
    env = dict(os.environ)
    env["OLLAMA_API_BASE"] = HEX_OLLAMA
    cmd = [
        str(AIDER_BIN),
        "ttl_cache.py", "test_ttl_cache.py",
        ...
        "--no-analytics",
        "--no-gitignore",
        "--map-tokens", "0",       # 2-file sandbox: no repo map needed
    ]
    t0 = time.monotonic()
...
def main() -> None:
    ...
    print(f"== aider smoke :: model=ollama_chat/{args.model} :: Hex={HEX_OLLAMA} ==")
    build_sandbox()
    ...
    print(f"== VERDICT [{args.model}]: {verdict} ==")
