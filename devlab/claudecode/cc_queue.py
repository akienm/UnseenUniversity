...
def _record_ticket_usage(ticket_id: str, ticket: dict, cost_usd: float | None = None) -> None:
    ...
    """
    try:
        from devlab.claudecode.usage_store import UsageStore
        store = UsageStore()
        store.record(
            ticket_id=ticket_id,
...
def _decision_rollup(tasks: list, decision_id: str) -> None:
    ...
    closes, un-gate dependents referencing this decision.

    The decision record itself is JSON in the canonical store
    (devlab/runtime/memory/decisions/); its close + outcome run through the
    /outcome flow, prompted by outcome_check.py on the last-ticket close. No
    rollup file is written here — the prior path created a stub under the
    now-retired lab tree (removed, T-cc-queue-rollup-misfiles-lab).
    """
    if not decision_id:
        return
    ...
    # All tickets in this decision are closed. The decision record is JSON in the
    # store; close + outcome run through the /outcome flow (prompted by
    # outcome_check.py). No rollup file is written here.
