...
### Who certifies + how the values get consumed (2026-06-20, continued with Akien)
- **Certifier = independent Evaluator (judge-panel) — not the builder, not the Critic.** Builder *produces evidence* + the mechanical proof (gate ran, red→green on the same test, commit-bound — re-verified by **code**, no inference). The Evaluator renders the **sufficiency** verdict ("could a hollow build pass this gate?") — the only non-mechanical part, and exactly the part the builder is biased to wave through (the builder is the demonstrated confabulator; a self-emitted sufficiency verdict = a fancier "done"). Not the **Critic**: it lives inside the build loop (Planner+Executor+Critic) → certifying its own collaboration is self-cert wearing a hat. Handoff is **bus-mediated** request/response (the rack gives it for free) — the autonomy story: no human, but an independent agent-of-record. Opposed incentives are why separation works even though the certifier is also an LLM: builder optimizes to close, certifier optimizes to catch hollowness, and the certifier judges the **artifact cold** (sees evidence + stated intention, never intent-in-head) so it can't be talked into the residue.
- **Three verdicts (not binary):** *certified-proven / rejected-hollow / **proven-to-best-current-ability***. The third is **CP1** — defeasible, commit-bound, timestamped; if a loose gate surfaces later that's **CP2** (revise + re-certify), not a scandal. Demanding *absolute* proof would either stall everything or force rubber-stamping; "proven as well as we currently know how, with the record attached" is the achievable, inspectable standard. The certification record is itself the turtle floor (inspectable, defeasible, not re-certified).
- **Cost is not an objection** (Akien): scope is narrow (load-bearing closes only), AND every call is a **learning signal (CP2)**. The proof+certification emissions are precisely the **corpus the eventual graph-trees compile from** — the expense buys what later makes it free. Panel depth scales with stakes (single independent judge = floor; full panel when contested/high-blast-radius). Don't run a panel on a prototype.
- **Trajectory (compiled-inference north star):** judge-panel handoff = the **interim** shape (builder passes the query *up*). Goal = graph-trees smart enough to **ask for help in understanding *before* doing**, instead of do-then-pass-up-for-judgment — orientation/understanding compiled to the **front**; the tree compiles "what the whole system is capable of." LATER step. **One small step at a time → build the certifier now; it generates the corpus the graph-trees learn from.**
- **Composite things (cardinality resolved → a tree):** a ticket can be a high-level thing that closes when **all children close AND its own proofs pass**. The parent's own proof must test the **emergent/integration** intention — the thing no child tests (children: "write works"/"read works"; parent: "round-trips") — **not** re-run the children. A parent whose "proof" is just "all children passed" is a *rollup, not a proof*, and would wave through the every-part-passes-but-the-whole-doesn't-integrate failure. One-falsifiable-intention-per-ticket still holds at every node (the parent's one intention = the integration claim).
- **Fit-To-Purpose (FTP) tests — the product-level proof above the ticket-level proof (OPEN THREAD, Akien 2026-06-21):** we have unit + some E2E tests, but no FTP tests — a test whose pass condition *is the higher-level intent*, framed from purpose not mechanics. E2E proves the system is *wired*; FTP proves the *purpose* is served. A green E2E with a failed FTP is the exact "zero tickets but it doesn't work / use-it test failed" gap that bit us every time we hit zero (see [[project_definition_of_done_tool_disappears]]). FTP is proof-on-close one altitude up: the test IS the intent operationalized — but at the **root of the tree** (the product's emergent reason-for-existing), not a ticket leaf. Connections: it **proves the MAP's intent** (map = claim, FTP = proof of claim, decision #5); its sufficiency is **Evaluator-judged** ("could it pass this and still not serve the purpose?" = the loose-test failure at product scale); it's **where top-level intent leaves Akien's head** (highest rung of the intention compiler). Caveats (CP1): partly judged/demonstrated, NOT fully automatable (be honest which is mechanical vs model-scored); must stay **falsifiable** or it rots to fluff ("delightful" ✗; "a new user given only the code can ask Igor to build an app for X and get a working app without touching internals" ✓). NOT yet designed/ticketed — its own design pass then /sorted.
- **Values consumption — RESOLVES decision #4's "CP2–CP6 consumption undefined":** the existing **tags** mechanism (ticket `tags` arrays + code `# tags:` headers — already pervasive for topic/area taxonomy) is the carrier. **CP1 consumes as a *gate* (the honesty gate). CP2–CP6 consume as *tags*** — every artifact (proof, certification, ticket, emission) carries the CP(s) it serves **as part of the *why***, making the values greppable/traceable/auditable everywhere = placement→consumption. Enforceable: `/audit-ticket` + `/skills-audit` can require a CP tag in the why. **CP1 precision:** CP-tagging is **not yet a live convention** — the tag *infra* exists, but the CP1–6 strings currently in the store are memory-node IDs in old cognition tickets, not value-tags. This is *extend the existing slot + systematize*, not "already done."
